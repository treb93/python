...
def flatten_map(map: np.ndarray) -> np.ndarray:
...
def map_to_vector(map: np.ndarray):
    ...
    flatten_position = np.argmax(map).astype(int)
    ...
    return grid_to_vector(flatten_position, map.shape[0])


def function_1(vector: np.ndarray, resolution: int) -> np.ndarray:
    grid_position = vector_to_grid(vector, resolution, False)
